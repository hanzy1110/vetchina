...
newtype BoW a = BoW
    {bowToMap:: M.Map Word' a} deriving (Show, Read)
...
freqToProb bow_@(BoW bow) = BoW $ M.fromList $ zip words probs where
...
data SpamModel = SpamModel {spamBow:: BoW Prob,
                            hamBow::BoW Prob} deriving Show
